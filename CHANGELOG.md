...
- Math methods §9 names the Grambsch–Therneau test and credits the tenure
  ...
  lists the Weibull fit's limited diagnostics among its Limitations, and says
  up front that the tool is a toolkit, for practitioners and their advisors.
...
- Math methods §8.4 and the user guide no longer say the intake rate and the
  restricted mean are positively correlated, nor that the expected-census
  interval is a lower bound on the uncertainty: the sign of their covariance
  is unknown. What is known is that overdispersed intakes make the interval
  too narrow.
...
- The user guide quoted a label for the capped fraction that no
  ...
  function it is. The Cox sheet is present when no predictor qualifies, holding
  a note, and both guides say so.
