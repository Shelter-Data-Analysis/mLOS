...
And yet the unified analyses "show" that dogs get harder to move the
longer they stay.
...
is increasingly made of long-stayers, and the *marginal* hazard declines
...
mean ≈ 1.44 × median, 90th percentile ≈ 3.32 × median. The mixed
sample breaks both, and `expected_km` pins the numbers:

| Unified statistic | Value | Memoryless benchmark |
|---|---|---|
| median | 8 days | — |
...
sample 0.886). mLOS reports exactly this fit as the **crude Weibull**
...
1. **A true null.** The two study periods are identical by
   ...
   both periods' KM statistics equal the same mixture truths. This case
   checks that the machinery reports *no effect* when there is none.
2. **The census composition fact.** Although LARGE dogs are 30% of
   ...
   120-day cap also leaves `n_capped` materially
   nonzero (13 stays).
...
- `expected.R` — the unified ratio violations, the null period HR, the
