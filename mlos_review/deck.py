...
def workload_slide(bundle: Bundle, vocab: Vocabulary, section: str,
    ...
    shared = [
        ...
        "takes the level's average intake rate and survival curve and asks "
        "what population they would settle at; the counted side simply "
        "remembers what happened. So a level counted ABOVE its fitted figure "
        "was carrying more than its own steady state would hold, and one "
        "counted below was carrying less. A change in intakes and a change in "
        "how fast stays end both open the gap, in either direction, so name "
        "the cause only if you know which happened. It is a dynamic reading "
        "in a deck that otherwise "
